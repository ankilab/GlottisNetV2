...
class DataGenerator(Sequence):
    ...
    def _get_augmenter(self):
        ...
        aug = A.Compose(
            ...
            # Add additional target --> has to be changed manually when number of frames is changes
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_size):
            ...
            # Allocate lists for images and masks
            img = []
            mask = []
            ...
            # Extract frame number up to which the frames are extracted
            nr = X_ids.iloc[i][2]
            
            # Extract frames from video
            start = nr-self.frame_nr
            end = nr
            imgs = np.asarray(img[start:end])
            masks = np.asarray(mask[start:end])
            ...
            if img_height != self.target_height or img_width != self.target_width:
                ...
                # Store resized images back into original format
                frames = new_fr
            ...
            # Augment image and adjust keypoints accordingly --> The additional images and frames have to be changed manually
            if self.augment:
            ...
            # Store augmented images back into original format
            ...
            # Store normalized images back into original format
            frames = new_fr1
            frames_seg = new_seg1
        ...
        # Two exits
        # First exit: predictions of several frames of anterior and posterior points (2 channels). The frames are
        # stored in an extra dimension
